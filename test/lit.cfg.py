...
tools = ["opt", "llvm-as", "lli", "not", "FileCheck", "clang", "ld.lld"]
...
if not Path(config.hello_pass_path).is_file():
    print(f"Pass not found: {hello_pass_path}")
    exit(1)

config.substitutions.append(
    ("%hello_pass_path", config.hello_pass_path)
)

if not Path(config.hello_lto_pass_path).is_file():
    print(f"Pass not found: {hello_lto_pass_path}")
    exit(1)

config.substitutions.append(
    ("%hello_lto_pass_path", config.hello_lto_pass_path)
)
